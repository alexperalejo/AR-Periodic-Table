...
public class OrbitRing : MonoBehaviour
{
    ...
    private void Update()
    {
        // Keep line width proportional to the atom's world scale.
        // LineRenderer widths are always in world-space units and do NOT scale
        // with the parent transform, so when the atom is shrunk (e.g. to 0.04
        // by ElementPopupOnGrab) the ring radius shrinks but width stays fixed,
        // making the ring look like a thick donut. Scaling here fixes it.
        if (lr != null)
        {
            float worldScale = Mathf.Abs(transform.lossyScale.x);
            lr.startWidth = lineWidth * worldScale;
            lr.endWidth   = lineWidth * worldScale;
        }
    }
