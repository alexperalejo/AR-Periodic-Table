...
public class ElementLoader : MonoBehaviour
{
    ...
    // Post-parse: fill AtomElementData.cpkHex by re-scanning the raw JSON with Newtonsoft.
    // Called immediately after JsonUtility.FromJson<PeriodicTable>(rawJson) because
    // JsonUtility cannot deserialize a field whose JSON key contains a dash ("cpk-hex").
